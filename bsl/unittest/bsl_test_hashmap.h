 ...
 * $Id: bsl_test_hashmap.h,v 1.6 2009/06/26 05:23:25 xiaowei Exp $ 
 *
...
bool test_hashmap_serialization()
{
	...
	//读硬盘
	{
		...
		for (std::map<std::string, std::string>::iterator iter = vec.begin();
				iter != vec.end(); ++iter) {
			std::string val = "";
...
public:
	...
	typedef bsl::hashmap<std::string, std::string, hash_key> THMp1;
	void test_operator() {
		{
			THMp1 ht;
			std::map<key,value> mp;
			ht.assign(mp.begin(),mp.end());
			THMp1 ht2;
			ht2 = ht;
			THMp1 ht3(ht);
		}
		{
			...
			for (size_t i = 0; i < N; i ++) {
				...
				value val2;
				value val3;
				int ret = ht.get(key(s[i]),&val);
				TS_ASSERT( ret == bsl::HASH_EXIST );
				ret = ht2.get(key(s[i]),&val2);
				TS_ASSERT( ret == bsl::HASH_EXIST );
				ret = ht3.get(key(s[i]),&val3);
				TS_ASSERT( ret == bsl::HASH_EXIST );
				TS_ASSERT( val2 == val3 );
				TS_ASSERT( val == val2 );
			}
	...
	**/
	...
	//bsl_alloc分配器
	typedef bsl::hashmap<std::string, std::string, hash_key, bsl::equal<std::string>, bsl::bsl_alloc<std::string> > THMp2;
	void test_hashmap2_() {
	...
	//默认的phashmap
	typedef bsl::phashmap<std::string, std::string, hash_key> THMp3;
	void test_phashmap_() {
	...
	//bsl_alloc的phashmap
	typedef bsl::phashmap<std::string, std::string, hash_key, bsl::equal<std::string>, bsl::bsl_alloc<std::string> > THMp4;
	...
	//bsl_cpsalloc分配器
	typedef bsl::phashmap<std::string, std::string, hash_key, bsl::equal<std::string>, bsl::bsl_cpsalloc<bsl::bsl_alloc<std::string> > > THMp5;
	...
	void test_phashmap3_clear_() {
		bsl::hashmap<int, std::string> v1;
		bsl::hashmap<int, bsl::string> v2;
		v1.create(100);
		v2.create(1000);
		v1.set(1, "hello");
		v1.get(1);

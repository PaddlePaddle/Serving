...
import sys
from network_conf_kv import dnn_model
...
def train():
    ...
    predict_y, loss, auc_var, batch_auc_var, inference_feed_vars = dnn_model(
    ...
    feed_var_dict_with_kv = {}
    feed_var_dict_with_kv['dense_input'] = dense_input
    for i, embedding in enumerate(inference_feed_vars):
        feed_var_dict_with_kv["embedding_{}".format(i)] = embedding
    fetch_var_dict_with_kv = {"prob": predict_y}
    ...
    server_io.save_model(
        "ctr_serving_model_with_kv", "ctr_client_conf_with_kv",
        feed_var_dict_with_kv, fetch_var_dict_with_kv, fluid.default_main_program())

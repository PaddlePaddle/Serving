...
def dnn_model(dense_input, sparse_inputs, label,
    ...
    emb_lists = list(map(embedding_layer, sparse_inputs))
    inference_feed_vars = [x[0] for x in emb_lists]
    emb_sums = [x[1] for x in emb_lists]
    ...
    return predict, avg_cost, auc_var, batch_auc_var, inference_feed_vars

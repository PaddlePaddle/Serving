...
int GeneralDistKVInferOp::inference() {
  ...
  for (size_t i = 0; i < in->size(); ++i) {
    ...
    for (size_t x = 0; x < sparse_out[sparse_idx].lod.size(); ++x) {
      ...
      std::copy(in->at(i).lod[x].begin(), in->at(i).lod[x].end(), sparse_out[sparse_idx].lod[x].begin());
    }
    ...
    sparse_out[sparse_idx].data.Resize(sparse_out[sparse_idx].lod[0].back() * EMBEDDING_SIZE * sizeof(float));
    //std::cout << "embedding byte size: " << sparse_out[sparse_idx].lod[0].back() * EMBEDDING_SIZE * sizeof(float) << std::endl;
  ...
  if (InferManager::instance().infer(GENERAL_MODEL_NAME, &infer_in, out, batch_size)) {
